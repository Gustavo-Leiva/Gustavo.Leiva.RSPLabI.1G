...
{
    int returnAux = -1;
    Node* nodoAux = NULL; //creo una variable del vagon a eliminar
    Node* nodoAux2 = NULL;

    if(this != NULL && index >= 0 && index < ll_len(this))
    {
        nodoAux = getNode(this, index);// es la posicion a donde apuntaba mi nodo que quiero eliminar
        if(index == 0)
        {
        ...
        {
            nodoAux2 = getNode(this, index -1); // adonde apunta el nodo anterior al q elimino q es el nodo a eliminar
...
int ll_deleteLinkedList(LinkedList* this)
{
    ...
    if(this != NULL)
    {
        if(!ll_clear(this)) //si devuelve 0 hago un free
        {
            free(this);
            returnAux = 0;
        }
 ...
 *
 * \param void* this la lista a filtrar
...
LinkedList* ll_map(LinkedList* lista, void* (*pFunc)(void* pElement))
{
    ...
    if(lista != NULL && pFunc != NULL)
    {
        ...
        if(mapArray != NULL)
        {
            ...
            {
                pFunc(ll_get(lista, i));
                ll_add(mapArray, ll_get(lista, i));
            }
        }
    }
    return mapArray;
